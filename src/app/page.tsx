...
export default function Dashboard() {
  ...
  const tasksQuery = useMemoFirebase(() => {
    ...
    // Получаем все задачи организации (без фильтров по статусу в запросе, чтобы избежать ошибок индексов и прав)
  ...
  const filteredTasks = useMemo(() => {
    ...
    // Скрываем завершенные для всех, кроме владельца (фильтрация на клиенте для стабильности)
    ...
    <Layout title="Задачи">
      <div className="space-y-6 max-w-5xl mx-auto">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative group flex-1">
            ...
            <Input 
              ...
              className="pl-10 h-12 bg-white border-slate-950 shadow-none rounded-none text-xs font-black uppercase tracking-widest focus-visible:ring-0"
              value={search}
          ...
          <div className="w-full md:w-80">
            <Select value={selectedDept} onValueChange={setSelectedDept}>
              <SelectTrigger className="h-12 bg-white border-slate-950 border-2 shadow-none rounded-none text-[10px] font-black uppercase tracking-widest text-slate-950 focus:ring-0">
                <div className="flex items-center gap-2">
                  <Building2 className="w-4 h-4" />
                  <SelectValue placeholder="ВСЕ ОТДЕЛЫ" />
                </div>
              </SelectTrigger>
              <SelectContent className="rounded-none border-2 border-slate-950">
        ...
        <div className="flex items-center justify-between border-b-2 border-slate-950 pb-2">
          ...
          <div className="text-[10px] font-black text-white bg-slate-950 px-3 py-1 uppercase">
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[1, 2, 3, 4, 5, 6].map(i => (
              <Skeleton key={i} className="h-40 w-full rounded-none bg-slate-200 border-2 border-slate-300" />
            ))}
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-32 bg-white border-2 border-slate-950 border-dashed">
            <div className="w-16 h-16 bg-slate-50 border-2 border-slate-950 flex items-center justify-center mb-4">
              <FolderKanban className="w-8 h-8 text-slate-950" />
            </div>
            <p className="text-slate-950 font-black text-[10px] uppercase tracking-[0.4em]">ЗАДАЧ НЕ НАЙДЕНО</p>
          </div>
        )}
      </div>
    </Layout>
